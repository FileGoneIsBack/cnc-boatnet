...
// availableAttackMethods returns a sorted slice of all loaded method names.
func AvailableAttackMethods() []string {
    methods := make([]string, 0, len(floods.Methods))
    for name := range floods.Methods {
...
// filter returns only those entries from list that start with prefix.
func Filter(prefix string, list []string) []string {
